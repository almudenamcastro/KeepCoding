...
```{r}
for (i in 1:nrow(airbnb_cols)){
  if (airbnb_cols$unique_values[i] < 3 | airbnb_cols$empty[i] > 11987){
    nombre <- airbnb_cols$column[i]
    airbnb_clean <- airbnb_clean |> 
      select(-nombre)
  }
...
airbnb_clean <- airbnb_clean |>
  ...
  select(-Host.Neighbourhood, -Host.Total.Listings.Count, -Calculated.host.listings.count, -Geolocation, -Market, -Smart.Location)
...
-   **Calendar.updated**: turn it into days_since last update to make it quantitative. (Although this metric might not be needed)
