...
for (i in 1:nrow(area_media)){
  print(paste("El área media de las semillas de tipo", area_media[[i,1]],
              "en mm cuadrados es", round(area_media[i,2],2)))
}
...
**Respuesta**: Para comprobar la validez del modelo. Si "testamos" un modelo con los mismos datos que hemos usado en el entrenamiento, corremos el riesgo de que el modelo solo de una respuesta correcta para esos datos, que ya conoce. Podría tratarse de un modelo sobre entrenado, en ese sentido: muy preciso para cierto conjunto de datos, pero demasiado rígido como para predecir otros resultados que se salgan de ahí.
...
**Respuesta**: 'Usaremos la función de regresión logística, que nos sirve para ajustar datos que siguen una distribución binaria (datos cuyo valor puede ser 0 o 1).
